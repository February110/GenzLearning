    ...
    <div className="p-0">
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-4">
          {items.map((a) => (
            <li key={a.id} className="rounded-2xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition p-4">
...
function AnnouncementFiles({ announcementId, initialItems }: { announcementId: string; initialItems?: any[] }) {
  ...
  if (items === null) return <div className="text-sm text-gray-500 p-2">Đang tải...</div>;
  if (items.length === 0) return <div className="text-sm text-gray-500 p-2">Không có tệp đính kèm.</div>;
  return (
...
function AttachmentChip({ url, name }: { url?: string; name?: string }) {
  ...
  return (
    ...
    >
      ...
      <span className="truncate text-sm text-indigo-700 group-hover:underline">{label}</span>
    </a>
  );
}
  ...
  return (
    <div className="mt-3 border-t border-gray-100 dark:border-gray-800 pt-2">
      ...
      <div className={`${expanded ? 'max-h-64 overflow-y-auto pr-1' : ''} mt-2 space-y-2`}>
        ...
        {items.length === 0 && <div className="text-xs text-gray-500">Chưa có nhận xét nào.</div>}
      </div>

      <form onSubmit={send} className="flex items-center gap-2 mt-2">
        <input value={text} onChange={(e)=>setText(e.target.value)} placeholder="Thêm nhận xét..." className="flex-1 rounded-full border px-4 py-2 text-sm bg-white dark:bg-zinc-950" />
