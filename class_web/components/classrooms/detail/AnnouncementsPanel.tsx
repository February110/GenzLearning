...
import { MoreHorizontal, ChevronDown, Clock, Repeat2, Megaphone, ArrowLeft, Users, X } from "lucide-react";
...
function ReuseAnnouncementModal({
  ...
  const stepLabel = step === "class" ? "Chọn lớp" : "Chọn bài đăng";
      ...
      <div className="relative mx-auto max-h-[88vh] w-full max-w-4xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-zinc-900">
        <div className="flex items-start justify-between gap-4 border-b border-gray-200 bg-slate-50 px-5 py-4 dark:border-gray-800 dark:bg-zinc-950/80">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <div className="text-base font-semibold text-gray-900 dark:text-gray-100">Đăng lại thông báo</div>
              ...
              <div className="mt-1 truncate text-xs text-gray-600 dark:text-gray-400">
                {step === "class" ? "Sử dụng lại các bài đăng trong lớp học bạn giảng dạy." : selectedClass?.name || "Lớp học"}
              </div>
            </div>
          </div>
          <button
        ...
        {step === "class" ? (
          ...
          <div className="p-5">
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-zinc-900">
              <div className="overflow-x-auto">
                <div className="min-w-[720px]">
                  <div className="grid grid-cols-[36px_1.6fr_1fr_0.9fr] gap-3 border-b border-gray-200 bg-gray-50 px-4 py-2.5 text-xs font-medium uppercase tracking-wide text-gray-500 dark:border-gray-800 dark:bg-zinc-950 dark:text-gray-400">
                    <div />
                    <div>Nội dung</div>
                    <div>Giáo viên</div>
                    <div>Ngày đăng</div>
                  ...
                  <div className="max-h-[360px] min-h-[240px] overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
                    {announcementsLoading ? (
                      ...
                      <div className="px-4 py-8 text-center text-sm text-gray-500">Lớp này chưa có thông báo.</div>
                    ) : (
                      announcements.map((a) => {
                        ...
                        return (
                          <label
                            key={a.id}
                            className={`grid cursor-pointer grid-cols-[36px_1.6fr_1fr_0.9fr] items-center gap-3 px-4 py-3 text-sm transition ${
                              selected
                          ...
                          >
                            ...
                            <div className="truncate text-xs text-gray-500 dark:text-gray-400">{dateLabel}</div>
                          </label>
                        );
                      })
                    )}
            ...
            <div className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-800 dark:bg-zinc-950">
              <label className="inline-flex min-w-0 items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                <input
                  type="checkbox"
                  className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                ...
                />
                <span className="truncate">Tạo bản sao mới cho tất cả các tệp đính kèm</span>
              </label>
              <Button type="button" size="md" className="!rounded-full" disabled={!selectedAnnouncementId} onClick={onUse}>
                Sử dụng lại
              </Button>
            </div>
          </div>
